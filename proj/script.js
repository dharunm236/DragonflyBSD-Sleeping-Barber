    ...
    setTimeout(async () => {
        ...
        for (let i = 0; i < customers.length; i++) {
            if (waitting < chairs.length) {
                ...
                waitting++;
                console.log(
                    `Patient Entered the Waiting Room ${customers[i]
                    } at ${new Date().toLocaleTimeString()}`
                );
                waitingRoom.innerHTML += `<div class="bg-blue-500 text-white py-2 px-4 rounded">Patient ${customers[i]
                ...
                if (queue.size() === 0) {
                    ...
                    console.log(
                        `Patient Left because of Full Waiting Room ${customers[i]
                        } at ${new Date().toLocaleTimeString()}`
                    );
                    leaveWaitingRoom.innerHTML += `<div class="bg-red-500 text-white py-2 px-4 rounded">Patient Left because of Full Waiting Room ${customers[i]
    ...
    setTimeout(async () => {
        if (waitting > 0) {
            for (let i = 0; i < waitting; i++) {
                ...
                console.log(
                    `Patient getiing treated ${customer} at ${new Date().toLocaleTimeString()}`
                );
                cuttingRoom.innerHTML += `<div class="bg-green-500 text-white py-2 px-4 rounded">Doctor treating  ${customer} at ${new Date().toLocaleTimeString()}</div>`;
                await sleep(2000);
                console.log(
                    `Treatmenr done ${customer} at ${new Date().toLocaleTimeString()}`
                );
                cuttingLeavingRoom.innerHTML += `<div class="bg-slate-800 text-white py-2 px-4 rounded">Patient ${customer} Left the clinic ${new Date().toLocaleTimeString()}</div>`;
                await sleep(2000);
                cuttingRoom.removeChild(cuttingRoom.childNodes[0]);
                console.log(
                    `Patient left clinic ${customer} at ${new Date().toLocaleTimeString()}`
                );
                if (i == waitting - 1) {
                    cuttingRoom.innerHTML += `<div class="bg-green-500 text-white font-bold py-2 px-4 rounded">Doctor room ${new Date().toLocaleTimeString()}</div>`;
                }
                await sleep(2000);
            }
        }
        if (waitting == 0 || waitting == chairs.length) {
            if (waitting == 0) {
                waitingRoom.innerHTML += `<div class="bg-blue-500 text-white font-bold py-2 px-4 rounded">Waiting Room Empty at ${new Date().toLocaleTimeString()}</div>`;
                cuttingRoom.innerHTML += `<div class="bg-green-500 text-white font-bold py-2 px-4 rounded">DOCTOR room Empty at ${new Date().toLocaleTimeString()}</div>`;
            }
            baberSl.style.display = "block";
            console.log(
                `Doctor Sleeping at ${new Date().toLocaleTimeString()} `
            );
            barberSleeping.innerHTML += `<div class="bg-yellow-500 text-white font-bold py-2 px-4 rounded">Doctor Sleeping at ${new Date().toLocaleTimeString()}</div>`;
        }
    }, 2000);
}